...
function webp (options) {
  ...
  if (is.object(options) && is.defined(options.pageHeight)) {
    if (is.integer(options.pageHeight) && options.pageHeight > 0) {
      this.options.pageHeight = options.pageHeight;
    } else {
      throw is.invalidParameterError('pageHeight', 'integer larger than 0', options.pageHeight);
    }
  }
  if (is.object(options) && is.defined(options.loop)) {
    if (is.integer(options.loop) && is.inRange(options.loop, 0, 65535)) {
      this.options.loop = options.loop;
    } else {
      throw is.invalidParameterError('loop', 'integer between 0 and 65535', options.loop);
    }
  }
  if (is.object(options) && is.defined(options.delay)) {
    if (
      Array.isArray(options.delay) &&
      options.delay.every(is.integer) &&
      options.delay.every(v => is.inRange(v, 0, 65535)))
    {
      this.options.delay = options.delay;
    } else {
      throw is.invalidParameterError('delay', 'array of integers between 0 and 65535', options.delay);
    }
  }
